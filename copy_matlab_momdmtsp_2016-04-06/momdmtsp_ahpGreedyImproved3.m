...
%Date: 11-11-2015
%tsp= lkh
% cost_table=horzcat(cost_table, cost_rtma_mo2(depots(ri,:), Targets,
% weight))

% cost_rtma modified version
function [rte, brk, globalCost]=momdmtsp_ahpGreedyImproved3(Targets, depots, compMat, show_prog)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
 ...
 cost_table=[];
 for ri=1:nbrRobots
     cost_table=horzcat(cost_table, cost_rtma_mo2(depots(ri,:), Targets, weight))
...
%            %compute the new time
           newmaxTime(rj)=max([time,time(rj)])
%            %new energy
           newEnergy(ri)=energy(ri)
%            %compute the new variance
%            x=NbATargets
%            x(r)=newNbATargets(r)
%            newVariance(r)=var(x)
