function varargout = mdmtsp_ga_multi_ch_mo(xy,salesmen,depots, weight, min_tour,max_tour,tw,pop_size,num_iter,use_complex,show_prog,show_res)
%mdmtspv_ga_mo (xy,max_salesmen,depots,CostType,weight,min_tour,pop_size,num_iter,show_prog,show_res,dmat)
    ...
    for p = 1:pop_size
        ...
        for s = 1:length(pop{p}.ch)
			...
			if ~isempty(sman)
                ...
                d2 = d2 + dmat(s, sman(end)); % Add End Distance
% 				if (d2 > max_tour)
...
% 				end
        ...
        total_dist(p) = wd*sum(cost)+wmt*max(cost);
    end
    ...
    for p = 8:8:pop_size
        ...
        for k = 1:8 % Generate New Solutions
            ...
            switch k
                ...
                case 5 % crossover
					if (sum(cellfun(@(c) ~isempty(c), tmp_pop_8)) > 1 && lbestch>1)
