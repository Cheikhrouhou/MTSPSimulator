...
function script_plotting_statistics_impact_improv_phase(ni, nf, ns, stp, dim, showPlot)
...
%     bar(cent(:,1),AvgGA(:,4))
    ...
    hold on
    errorbar(AvgGA(:,1),AvgGA(:,5),ErGA(:,5),'--','Color',clr(2,:))
    errorbar(AvgGA(:,1),AvgGA(:,6),ErGA(:,6),'-','Color',clr(2,:))
    
    hold on
    errorbar(AvgGA(:,1),AvgGA(:,7),ErGA(:,7),'--','Color',clr(3,:))
    errorbar(AvgGA(:,1),AvgGA(:,8),ErGA(:,8),'-','Color',clr(3,:))
    
    legend('Market Based (Before Improvement)','Market Based (After Improvement)',...
        'RTMA (Before Improvement)','RTMA (After Improvement)', ...
        'Balanced (Before Improvement)', 'Balanced (After Improvement)')
    %legend('boxoff')
