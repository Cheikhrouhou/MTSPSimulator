...
% modified computing of cost in the greedy phase

function [rte, brk, globalCost]=momdmtsp_ahpGreedyImproved4(Targets, depots, compMat, show_prog)
...
for ri=1:nbrRobots
    ...
    if (changeFlag(ri))
        ...
        candtour{ri}=[];
        changeFlag(ri)=0;
        rtma{ri}=cost_rtma(depots(ri,:), UTargets);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
 ...
 for ri=1:nbrRobots
     ...
     cost_table=horzcat(cost_table, cost_rtma_mo(depots(ri,:), Targets, weight))
 end
