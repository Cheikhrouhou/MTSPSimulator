...
% we make call to greedy approach
[rte, brk, greedy_cost]=mdmtsp_greedy_mo(xy, depots,weight, 1)
%
...
for k = 1:pop_size
    switch k
        ...
        case 2
            %we change only brk
            pop_rte(k,:)=[rte{1:end}]
            step=round(n/max_salesmen);
            pop_brk{k}=[1:step:n]
            pop_brk{k}=pop_brk{k}(1:max_salesmen)
        otherwise
             pop_rte(k,:) = randperm(n);
             pop_brk{k} = randbreak(max_salesmen,n);
    end
end
...
if show_prog
    pfig = figure('Name','MDMTSP_GA | Current Best Solution','Numbertitle','off');
end
iter=0;
iter2go=0;
while iter2go < num_iter
    ...
    for p = 1:pop_size
        ...
        elseif CostType==2
            ...
            %multi-objective global cost
            TTD(p)=sum(d);
            MT(p)=max(d);
            total_dist(p)=wd*sum(d)+wmt*max(d);
        end
    end
    ...
    if min_dist < global_min
        ...
        if show_prog
            ...
            for s = 1:salesmen
                ...
                try
                    title(sprintf(['Global Cost= %1.2f TTD = %1.4f, MT=%1.2f Salesmen = %d, ' ...
                    'Iteration = %d'],min_dist,TTD(index), MT(index), salesmen,iter));
                catch
    ...
    % Genetic Algorithm Operators==>generate new population
        ...
        for k = 1:ops % Generate New Solutions
            ...
            switch k
                ...
                case 5 % Change Breaks
                    tmp_pop_brk{k} = randbreak(max_salesmen,n);
...
function breaks = randbreak(max_salesmen,n)
